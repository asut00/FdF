...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/31 14:55:26 by asuteau           #+#    #+#             */
/*   Updated: 2024/05/31 15:13:39 by asuteau          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
								...
								t_duo *duo, t_map_var *var)
{
	...
	while (jv->ptd_x != duo->iso_x2 && jv->ptd_y != duo->iso_y2)
	{
		...
		col_y_handle_lower_y2_loop(jv, p_img, duo, var);
		jv->ptd_x--;
...
void	col_x_handle_else(t_jvar *jv, t_data *p_img, t_duo *duo, t_map_var *var)
{
	jv->rest = 1;
	if (jv->abs_y_dist)
	...
	while (jv->ptd_x != duo->iso_x2 && jv->ptd_y != duo->iso_y2)
	{
		...
		col_x_handle_else_loop(jv, p_img, duo, var);
								...
								t_duo *duo, t_map_var *var)
{
		...
		&& jv->ptd_y != duo->iso_y2 && jv->moves_per_ldu > 0)
	{
		...
		jv->moves_per_ldu--;
		if (jv->rest
			&& jv->dist_p >= (((jv->abs_x_dist * jv->k) / jv->rest)))
		{
			jv->ptd_x--;
			jv->dist_p++;
			ft_pput(p_img, jv->ptd_x, jv->ptd_y,
